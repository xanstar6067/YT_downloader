...
- сохранение последней папки, режима, качества, режима плейлиста и темы в `%LOCALAPPDATA%\YtDlpDownloader\settings.json`;
...
6. Машиночитаемый UTF-8-вывод `--progress-template` преобразуется в состояние интерфейса; для плейлиста учитываются текущая позиция и общее количество элементов. При HTTP 403 приложение один раз перезапускает yt-dlp, чтобы получить свежие медиассылки. При отмене дочерний процесс завершается целиком.
