...
6. Машиночитаемый UTF-8-вывод `--progress-template` преобразуется в состояние интерфейса. Раздельные видео- и аудиопотоки отображаются как последовательные этапы единой шкалы; для HLS/DASH используется позиция фрагмента, поэтому служебный стартовый фрагмент не создаёт ложный скачок до 50%. Для плейлиста дополнительно учитываются текущая позиция и общее количество элементов. Если прогресс определить нельзя, шкала переходит в анимированный режим. При HTTP 403 приложение один раз перезапускает yt-dlp, сохраняя монотонный прогресс. При отмене дочерний процесс завершается целиком.
